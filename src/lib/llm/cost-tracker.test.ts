...
describe("calculateCost", () => {
  it("returns 0 for zero tokens", () => {
    expect(calculateCost("claude-sonnet-4-5-20250929", 0, 0)).toBe(0);
  });

  it("calculates Opus input cost: 1M tokens at $15/M = $15", () => {
    const cost = calculateCost("claude-opus-4-5-20250514", 1_000_000, 0);
    expect(cost).toBe(15);
  });

  it("calculates Opus output cost: 1M tokens at $75/M = $75", () => {
    const cost = calculateCost("claude-opus-4-5-20250514", 0, 1_000_000);
    expect(cost).toBe(75);
  });

  it("calculates Sonnet mixed cost correctly", () => {
    // 500k input at $3/M + 500k output at $15/M = $1.50 + $7.50 = $9
    const cost = calculateCost("claude-sonnet-4-5-20250929", 500_000, 500_000);
    expect(cost).toBe(9);
  });
  ...
  it("scales linearly with token count", () => {
    const cost1 = calculateCost("claude-sonnet-4-5-20250929", 100_000, 100_000);
    const cost2 = calculateCost("claude-sonnet-4-5-20250929", 200_000, 200_000);
    expect(cost2).toBeCloseTo(cost1 * 2);
  });

  it("handles small token counts with precision", () => {
    // 1000 input tokens at $3/M = $0.003
    const cost = calculateCost("claude-sonnet-4-5-20250929", 1000, 0);
