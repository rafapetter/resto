...
export function DashboardPhase({ isPlaying, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    // After all cards visible, highlight Chat card then advance
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold">Bella&apos;s Bistro</h1>
          <Badge variant="secondary">Active</Badge>
        </div>
        <p className="mt-1 text-muted-foreground">
          Italian restaurant with online ordering, reservations, menu
          management, and loyalty program
        </p>
      </div>
        ...
        {sections.map((section, i) => (
          <Card
            ...
            className={cn(
              ...
              highlightChat && section.title === "Chat"
                ? "ring-2 ring-emerald-600 shadow-lg"
                : "hover:shadow-md"
            )}
            onClick={() => {
              if (section.title === "Chat") onComplete();
            }}
          >
            <CardHeader>
      ...
      {highlightChat && (
        <p className="mt-4 text-center text-sm text-muted-foreground animate-pulse">
          Click Chat to start talking with Resto...
        </p>
      )}
    </div>
  );
